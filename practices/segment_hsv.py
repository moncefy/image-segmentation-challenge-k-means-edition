...
print("Using HSV features (3 features per pixel)")
...
for idx, image_file in enumerate(image_files, 1):
    ...
    pixel_vals = hsv_image.reshape((-1, 3)).astype(np.float32)
    ...
    for k in Ks:
        ...
        centers_hsv = np.uint8(centers).reshape(1, -1, 3)
        centers_rgb = cv2.cvtColor(centers_hsv, cv2.COLOR_HSV2RGB)
